...
def test_query_iterator_db_change(milvus_client: MilvusClient):
    milvus_client.use_database(db_1)
    # test query iterator
    expr = f"10 <= {AGE} <= 25"
    ...
    while True:
        ...
        for i in range(len(res)):
            print(res[i])
        milvus_client.use_database(db_2)
        page_idx += 1
        print(f"page:{page_idx}-------------------------")


def test_search_iterator_db_change(milvus_client: MilvusClient):
    milvus_client.use_database(db_1)
    # test query iterator
...
def main():
    ...
    for db in [db_1, db_2]:
        milvus_client.create_database(db)
        milvus_client.use_database(db)
    ...
    #test_query_iterator_db_change(milvus_client=milvus_client)
    test_search_iterator_db_change(milvus_client=milvus_client)
