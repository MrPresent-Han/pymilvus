...
HOST = "localhost"
PORT = "19530"
COLLECTION_NAME_1 = "test_iterator_1"
COLLECTION_NAME_2 = "test_iterator_2"
USER_ID = "id"
MAX_LENGTH = 65535
AGE = "age"
DEPOSIT = "deposit"
...
def re_create_collection(prepare_new_data: bool):
    for collection_name in [COLLECTION_NAME_1, COLLECTION_NAME_2]:
        if prepare_new_data:
            ...
            Collection(collection_name, schema, consistency_level=CONSISTENCY_LEVEL, num_shards=2)
...
def query_iterate_collection_no_offset(collection):
    ...
    while True:
        ...
        print(f"got pk_cursor:{cursor.int_pk}")
        for i in range(len(res)):
            print(res[i])
...
def query_iterate_collection_without_offset(collection):
    query_iterator = collection.query_iterator(output_fields=[USER_ID, AGE],
                                               batch_size=50, consistency_level=CONSISTENCY_LEVEL)
    page_idx = 0
    while True:
        res = query_iterator.next()
        if len(res) == 0:
