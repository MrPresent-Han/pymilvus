...
def generate_ordered_strings(number, length):
    str_number = str(number).zfill(length)
    return str_number
...
fields = [
    ...
    FieldSchema(name="str_1000", dtype=DataType.VARCHAR, max_length=1000)
]
...
if prepare_data:
    for i in range(batch_num):
        data = [
            ...
            [i for i in range(nb * i, nb * (i + 1))],
            vectors,
            [generate_ordered_strings(i, 10) for i in range(nb * i, nb * (i + 1))],
            [generate_ordered_strings(i, 100) for i in range(nb * i, nb * (i + 1))],
            [generate_ordered_strings(i, 1000) for i in range(nb * i, nb * (i + 1))],
        ]
